...
1. **Canonical checkout is a parking spot**: configured `repos.<name>.path` checkouts are not where work happens. Runtime command guards and installed hooks both protect this discipline: `guard_not_canonical` refuses user commands from canonical paths, `repo_for_cwd` classifies canonical versus worktree context, and the generated hook path cache records canonical paths for hook-time refusal (`generate_path_cache`). Read-only inspection commands (`wt list`, `wt status`, `wt cd`) are intentionally exempt and run from canonical; only mutating commands (`new`, `adopt`, `move`, `pr`, `reap`, `focus`, `tidy`) refuse.
...
3. **Branch policy has one runtime surface**: repo inference, quiet branch-policy predicates, user-facing validation, and hook-cache emission go through `branch_policy_match_repo`, `branch_policy_allows`, `branch_policy_validate`, and `branch_policy_emit_cache`. `wt new` and `wt move` validate through that surface before creating a worktree. Hook enforcement is opt-in via `hooks.enforce_branch_names` and uses the same quiet predicate as runtime validation.
4. **Config is a snapshot, not scattered yq reads**: `_cfg_load` performs one props dump and fills bash arrays. `cfg_repo_record` / `cfg_each_repo_record` are the stable internal record interface for repo config after defaults and `{repo}` expansion. Config writes must call `cfg_reload` before reading updated values (`cmd_onboard`). The generated hook cache must stay sourceable by macOS `/bin/bash` 3.2 because Git can invoke hooks outside Homebrew's modern Bash; do not use associative arrays in `paths.cache`. The snapshot also carries `worktree_symlinks` (a list, parsed like `branch_patterns` into `_CFG_SYMLINKS`, read via `cfg_worktree_symlinks`) and `setup_command` (a scalar needing no new parser case; read via `cfg_repo_get`); `cmd_new` consumes both to seed new worktrees.
...
7. **Hook chain composes instead of owning the repo**: wt-managed global hooks do wt's safety checks, then delegate to `~/.git-hooks-personal/<hook>`, then to repo-local `.git/hooks/<name>` as described in `README.md`. This allows guardrails, Husky, lefthook, pre-commit, and custom repo hooks to coexist without git-wt knowing their internals. The guard logic also has one self-contained surface in the binary, `wt hook-run <hook>` (pure, fail-open: canonical-commit refusal, single-session worktree ownership, opt-in branch validation), which `wt install-hooks` wires into a per-repo dispatcher when a repo's local `core.hooksPath` would otherwise bypass wt's global hooks; the dispatcher runs `wt hook-run` then chains the repo's original hooks, and `wt uninstall-hooks` reverts it. Composition over replacement still holds: the dispatcher never drops the team's hooks, and the guard never blocks when wt is absent/broken.
8. **Reaping is explicit and conservative**: `cmd_reap` refuses clean-up unless `worktree_reap_refusals` finds no safety refusals: the worktree must be clean, pushed, reachable from `origin/<default_branch>`, and have no open files, unless `--force` is supplied. There is no background auto-cleanup.
...
- **Branch policy**: runtime branch creation and hook-time validation need identical semantics. `branch_policy_*` is a real seam because two enforcement paths consume it.
  `branch_policy_allows` is the quiet predicate for hook paths; `branch_policy_validate` wraps it with user-facing errors for commands.
  `branch_policy_emit_cache` owns the hook-cache serialization.
- **Worktree state**: `status`, `tidy`, and `reap` all need the same clean/pushed/reachable semantics. `worktree_state_record` and `worktree_reap_refusals` keep display and safety checks from reimplementing that composition.
